...
SELECT cohort_month, week_number, active_users, cohort_size, retention_rate_percentage
...
SELECT 
    d.event_date,  -- 명시적으로 d (dau)의 event_date 사용
    d.dau,        -- dau의 dau 컬럼
    w.wau,        -- WAU의 wau 컬럼
    m.mau         -- mau의 mau 컬럼
FROM ECOMMERCE_DATA.STAGING_DATA.dau AS d
LEFT JOIN ECOMMERCE_DATA.STAGING_DATA.WAU AS w
    ON d.event_date = w.event_date
LEFT JOIN ECOMMERCE_DATA.STAGING_DATA.mau AS m
    ON d.event_date = m.event_date;
...
SELECT
    m.event_date,      -- mau에서 event_date 선택
    m.mau,             -- mau의 mau 값
    s.stickiness       -- stickiness의 stickiness 값
FROM ECOMMERCE_DATA.STAGING_DATA.mau AS m
JOIN ECOMMERCE_DATA.STAGING_DATA.stickiness AS s
    ON m.event_date = s.event_date;
