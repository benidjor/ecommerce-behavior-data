...
CREATE OR REPLACE STREAM FACT_CATEGORY_PRODUCT_TIME_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.FACT_CATEGORY_PRODUCT_TIME
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM RETENTION_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.RETENTION
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM WEEKLY_RETENTION_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.WEEKLY_RETENTION
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM MONTHLY_RETENTION_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.MONTHLY_RETENTION
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM dau_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.dau
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM WAU_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.wau
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM mau_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.mau
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM stickiness_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.stickiness
  APPEND_ONLY = TRUE;

CREATE OR REPLACE STREAM Segmented_RFM_stream
  ON TABLE ECOMMERCE_DATA.STAGING_DATA.Segmented_RFM
  APPEND_ONLY = TRUE;

-- 스트림 데이터를 한 번 조회하여 임시 테이블에 저장
CREATE OR REPLACE TEMPORARY TABLE time_dim_stream_tmp AS
SELECT *
FROM ECOMMERCE_DATA.PROCESSED_DATA.time_dim_processed_data_stream;

CREATE OR REPLACE TEMPORARY TABLE category_dim_stream_tmp AS
SELECT *
FROM ECOMMERCE_DATA.PROCESSED_DATA.category_dim_processed_data_stream;

CREATE OR REPLACE TEMPORARY TABLE product_dim_stream_tmp AS
SELECT *
FROM ECOMMERCE_DATA.PROCESSED_DATA.product_dim_processed_data_stream;

CREATE OR REPLACE TEMPORARY TABLE user_dim_stream_tmp AS
SELECT *
FROM ECOMMERCE_DATA.PROCESSED_DATA.user_dim_processed_data_stream;

CREATE OR REPLACE TEMPORARY TABLE fact_stream_tmp AS
SELECT *
FROM ECOMMERCE_DATA.PROCESSED_DATA.fact_processed_data_stream;

-- FACT 테이블과 CATEGORY_DIM, PRODUCT_DIM, TIME_DIM 테이블 JOIN

INSERT INTO ECOMMERCE_DATA.STAGING_DATA.FACT_CATEGORY_PRODUCT_TIME
...
FROM fact_stream_tmp AS f
LEFT JOIN category_dim_stream_tmp AS c
    USING (category_id)
LEFT JOIN product_dim_stream_tmp AS p
    USING (product_id)
LEFT JOIN time_dim_stream_tmp AS t
    USING (time_id);

-- RETENTION 테이블 생성

INSERT INTO ECOMMERCE_DATA.STAGING_DATA.RETENTION
WITH first_event AS (
    SELECT 
        f.user_id,
...
INSERT INTO ECOMMERCE_DATA.STAGING_DATA.WEEKLY_RETENTION
WITH first_event AS (
    SELECT 
        user_id, 
...
INSERT INTO ECOMMERCE_DATA.STAGING_DATA.MONTHLY_RETENTION
WITH first_event AS (
    SELECT 
        f.user_id,
...
INSERT INTO ECOMMERCE_DATA.STAGING_DATA.DAU
SELECT 
...
INSERT INTO ECOMMERCE_DATA.STAGING_DATA.WAU
SELECT 
    CAST(
        DATEADD(
          day, 
...
INSERT INTO ECOMMERCE_DATA.STAGING_DATA.MAU
SELECT 
...
-- Stickness (DAU / MAU)
INSERT INTO ECOMMERCE_DATA.STAGING_DATA.STICKINESS
WITH daily AS (
    SELECT 
        CAST(event_time AS DATE) AS event_date,
...
-- RFM 분석용 테이블
INSERT INTO ECOMMERCE_DATA.staging_data.Segmented_RFM
WITH RFM_Data AS (
    SELECT
        USER_ID,
